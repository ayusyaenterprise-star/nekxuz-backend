...
const LoginModal = ({ isOpen, onClose, onGoogleLogin }) => {
    ...
    const handleEmailLogin = async (e) => {
        ...
        try {
            ...
            // Save credentials for easy re-login
            localStorage.setItem('nekxuzAutoEmail', email);
            localStorage.setItem('nekxuzAutoPassword', password);
            onClose();
        } catch (err) {
            console.error(err);
            // Auto-create specific vendor account if not found
            ...
            setError("Invalid email or password. Create account or use Google.");
        } finally {
            setIsLoading(false);
        }
    };

    // Auto-fill from Google signup
    React.useEffect(() => {
        const savedEmail = localStorage.getItem('nekxuzGoogleEmail');
        if (savedEmail && !email) {
            setEmail(savedEmail);
            setGoogleSignupEmail(savedEmail);
            localStorage.removeItem('nekxuzGoogleEmail');
        }
    }, []);
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl relative">
                ...
                <div className="p-8 flex flex-col items-center text-center">
                    ...
                    <p className="text-sm text-gray-500 mb-8">Access wholesale pricing, track orders & connect with suppliers</p>
                    
                    {/* Google Login First */}
                    <button 
                        onClick={onGoogleLogin}
                        disabled={isLoading}
                        className="w-full flex items-center justify-center gap-3 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700 disabled:opacity-50 text-white font-bold py-3 px-4 rounded-xl transition-all active:scale-95 shadow-md mb-6"
                    >
                    ...
                    <div className="relative w-full mb-6">
                        ...
                        <div className="relative flex justify-center text-xs uppercase"><span className="bg-white px-2 text-gray-400">Or use email</span></div>
                    </div>
                    ...
                    <form onSubmit={handleEmailLogin} className="w-full space-y-4 mb-4">
                        ...
                        <div>
                            <input 
                                ...
                                className="w-full px-4 py-3 rounded-xl border border-gray-300 focus:border-secondary focus:ring-2 focus:ring-secondary/20 outline-none text-sm transition-all"
                                value={email}
                        ...
                        <div className="relative">
                            <input 
                                ...
                                className="w-full px-4 py-3 pr-10 rounded-xl border border-gray-300 focus:border-secondary focus:ring-2 focus:ring-secondary/20 outline-none text-sm transition-all"
                                value={password}
                        ...
                        {/* Error Message */}
                        {error && <p className="text-xs text-red-500 text-left bg-red-50 p-2 rounded">{error}</p>}

                        {/* Submit Button */}
                        <button 
                            type="submit" 
                            disabled={isLoading}
                            className="w-full bg-gradient-to-r from-secondary to-red-600 hover:from-red-600 hover:to-red-700 disabled:opacity-50 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-secondary/20"
                        >
                    ...
                    <p className="text-[10px] text-gray-400 mt-4">
                        💡 Tip: After Google signup, your email auto-fills here
                    </p>
                </div>
            </div>
        </div>
  ...
  const handleLogin = async () => {
    if (auth) {
      try {
        ...
        // Save Google email for auto-fill on next login
        if (result.user.email) {
          localStorage.setItem('nekxuzGoogleEmail', result.user.email);
        }
